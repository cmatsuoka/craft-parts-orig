...
from craft_parts.repo import apt_ppa, apt_sources_manager, errors
from craft_parts.repo.package_repository import (
...
def mock_apt_ppa_get_signing_key():
    with mock.patch(
        "craft_parts.repo.apt_ppa.get_launchpad_ppa_key_id",
...
@pytest.fixture(autouse=True)
def mock_host_arch():
    with mock.patch("craft_parts.step_info.StepInfo.deb_arch") as m:
...
def mock_sudo_write():
    ...
    with mock.patch("craft_parts.repo.apt_sources_manager._sudo_write_file") as m:
...
def mock_version_codename():
    with mock.patch(
        "craft_parts.utils.os_utils.OsRelease.version_codename",
...
def test_sudo_write_file(mock_unlink, mock_tempfile, mock_run):
    mock_tempfile.return_value.__enter__.return_value.name = "/tmp/foobar"

    apt_sources_manager._sudo_write_file(
        dst_path=Path("/foo/bar"), content=b"some-content"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "craft_parts-http_test_url_ubuntu.sources",
            dedent(
                """\
                Types: deb deb-src
        ...
        (
            ...
            "craft_parts-IMPLIED-PATH.sources",
            dedent(
                """\
                Types: deb
                URIs: http://test.url/ubuntu
        ...
        (
            ...
            "craft_parts-ppa-test_ppa.sources",
            dedent(
                """\
                Types: deb
...
def test_install_ppa_invalid(apt_sources_mgr):
    ...
    assert exc_info.value._ppa == "ppa-missing-slash"  # type: ignore
