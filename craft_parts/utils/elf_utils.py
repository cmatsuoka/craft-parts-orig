...
"""Helpers and utilities for ELF files."""
...
import elftools.common.exceptions  # type: ignore
import elftools.elf.constants  # type: ignore
import elftools.elf.dynamic  # type: ignore
import elftools.elf.elffile  # type: ignore
import elftools.elf.gnuversions  # type: ignore
import elftools.elf.sections  # type: ignore
import elftools.elf.segments  # type: ignore
from elftools.construct import ConstructError  # type: ignore
...
def ldd(path: str, ld_library_paths: List[str]) -> Dict[str, str]:
    ...
    logger.debug("invoking ldd with ld library paths: %s", ld_library_paths)
    ...
    try:
        ...
        logger.debug("ldd output:\n%s", ldd_lines)
    except subprocess.CalledProcessError:
        logger.warning("Unable to determine library dependencies for %s", path)
        return libraries
    ...
    logger.debug("ldd results: %s", libraries)
    return libraries
...
class Library:
    ...
    def __init__(
        ...
        *,
        ...
        logger.debug(
            "%s with original path %s found on %s in base: %s",
            soname,
            soname_path,
            self.path,
            self.in_base_snap,
    ...
    def _crawl_for_path(self) -> str:
        ...
        logger.debug("Crawling to find soname %s", self.soname)
...
class ElfFile:
    ...
    def __init__(self, *, path: str) -> None:
        ...
        self._required_glibc: Optional[str] = None
    ...
    def is_elf(cls, path: str) -> bool:
        """Verify whether the given file is an ELF binary.
    ...
    def _extract_attributes(self) -> None:  # noqa: C901
        with open(self.path, "rb") as elf_file:
    ...
    def is_linker_compatible(self, *, linker_version: str) -> bool:
        ...
        res = parse_version(version_required) <= parse_version(linker_version)
        logger.debug(
            "Checking if linker %s will work with GLIBC_%s required by %s: %s",
            linker_version,
            version_required,
            self.path,
            res,
    ...
    def get_required_glibc(self) -> str:
        ...
        if self._required_glibc is not None:
    ...
    def load_dependencies(
        ...
        logger.debug("Getting dependencies for %s", self.path)
        ...
        for path in search_paths:
            ld_library_paths.extend(
                os_utils.get_library_paths(root=path, arch_triplet=arch_triplet)
            )
...
def get_elf_files(root: str, file_list: Sequence[str]) -> FrozenSet[ElfFile]:
    ...
    for part_file in file_list:
        ...
        if os.path.islink(path):
            logger.debug("Skipped link %s while finding dependencies", path)
            continue
...
def clear_execstack(*, elf_files: FrozenSet[ElfFile]) -> None:
    """Clears the execstack for the relevant elf_files

    :param elf_files: the full list of elf files to analyze
                      and clear the execstack if present.
    """
    execstack_path = os_utils.get_snap_tool_path("execstack")
    ...
    for elf_file in elf_files_with_execstack:
        try:
            ...
            logger.warning("Failed to clear execstack for %s", elf_file.path)
