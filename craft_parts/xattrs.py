...
def _read_xattr(path: str, key: str) -> Optional[str]:
    ...
    try:
        ...
        raise errors.XAttributeError(action="read", key=key, path=path) from error
...
def _write_xattr(path: str, key: str, value: str) -> None:
    ...
    try:
        ...
        if error.errno == 7:
            raise errors.XAttributeTooLongError(path=path, key=key, value=value)

        # Chain unknown variants of OSError.
        raise errors.XAttributeError(action="write", key=key, path=path) from error
