...
def split_ppa_parts(*, ppa: str) -> Tuple[str, str]:
    """Split a ppa line into owner and name."""
...
def get_launchpad_ppa_key_id(*, ppa: str) -> str:
    ...
    logger.debug("Loading launchpad url: %s", launchpad_url)
    try:
    ...
    logger.debug("Retrieved launchpad PPA key ID: %s", key_id)
    return key_id
